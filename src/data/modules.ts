...
export class Modules {
  ...
  calculate_specs() {
    this.specs.clear();
    for (const i of Object.values(SpecType)) this.specs.set(i, 0);
    for (const mod of this.modules) {
      this.specs.set(mod.spec, (this.specs.get(mod.spec) || 0) + mod.kind.work_area);
    }
  }
...
export function getNeighbors(
  ...
  for (const other of modules_obj.modules) {
    ...
    if (okAB && okBA) out.push({ neighbor: other, sideOnMod, sideOnNei });
  }
  return out;
}
...
export function getBackNeighborForModule2(
  ...
  for (const other of modules_obj.modules) {
    ...
    if (other.position.distanceTo(backPos) < eps) {
      const delta = new Vector3().subVectors(other.position, mod2.position);
      const sideOnMod = sideFromDelta(delta);
